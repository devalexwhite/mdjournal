...
export default function MdDropzone({
  extensions,
  isLoggedIn,
  onDropFiles,
}: {
  ...
  return (
    <div>
      {files.length == 0 && (
        <Dropzone
          onDrop={onDrop}
          accept={extensions as any}
          onDragEnter={(_) => setDragging(true)}
          onDragLeave={(_) => setDragging(false)}
        >
          {({ getRootProps, getInputProps }) => (
            <div
              className={`w-full py-16 border border-dashed border-gray-700 dark:border-gray-100 px-6 ${
            ...
            >
              ...
              <div className="flex flex-row items-center">
                ...
                <h3 className="font-medium text-lg text-black dark:text-white">
                  {dragging
                    ? "now drop it!"
                    : `drop an .md or .txt file to ${
                        isLoggedIn ? "update" : "start"
                      } your blog.`}
                </h3>
              </div>
            </div>
