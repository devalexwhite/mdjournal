...
export default function Post({ fileUrl }: { fileUrl: string }) {
    ...
    const getMarkdown = async () => {
        const content = await supaclient.storage.from("posts").download(`${fileUrl}`);
        const text = await content.data?.text()

        const processedContent = await (await remark().use(html).process(text as any)).toString()
    ...
    return (
        <div>
            ...
            {
                !loading && (
                    <div className='animate-in prose lg:prose-xl' dangerouslySetInnerHTML={{ __html: markdown }}></div>
                )
            }
        </div>
    )
